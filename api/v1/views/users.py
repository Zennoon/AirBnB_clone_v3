...
def get_user_by_id(user_id):
    ...
    Retrieves a single User instance with the given ID, if it exists
    """
...
def delete_user_by_id(user_id):
    ...
    Deletes a User instance with the given ID from storage, if it
    exists
    """
    user = storage.get(User, user_id)
    if user is None:
...
def update_user_by_id(user_id):
    ...
    Modifies a User instance with given ID using attributes from
    request body
    """
    user = storage.get(User, user_id)
    if user is None:
